...
export class ValidationService {
   ...
   */
  static validateTravelRequest(request: TravelRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (!request.budget || !['low', 'medium', 'high'].includes(request.budget)) {
    ...
    if (request.travelers) {
      ...
      if (!travelersText.match(/\d+/) && !travelersText.includes('adult') && !travelersText.includes('person')) {
        errors.push('Travelers must include number information (e.g., "2 adults", "1 person")');
      }
    }
  ...
  static isSupportedDestination(destination: string): boolean {
    const supportedKeywords = [
      ...
      'pune', 'jaipur', 'agra', 'goa', 'kerala', 'rajasthan', 'uttar pradesh',
      'maharashtra', 'karnataka', 'tamil nadu', 'west bengal', 'gujarat',
      'udaipur', 'varanasi', 'rishikesh', 'shimla', 'ooty', 'mysore',
      'amritsar', 'leh', 'ladakh', 'kochi', 'madurai'
    ];

    const normalizedDestination = destination.toLowerCase();
    
    return supportedKeywords.some(keyword => 
      normalizedDestination.includes(keyword) || keyword.includes(normalizedDestination)
    );
  }
